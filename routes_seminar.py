...
def get_seminar_list():
    ...
    try:
        with sv.get_db_connection(sv.KNOWLEDGE_DB) as conn:
            cursor = conn.cursor()
            # lecture_audio_url を追加で取得し、プログラム内で判定を行う
            cursor.execute("""
                SELECT 
                    id, 
                    topic_title, 
            ...
            for row in cursor.fetchall():
                ...
                # PDFパス または 音声パス のいずれかが存在するかチェック
                has_pdf = bool(item.get('textbook_path') and str(item['textbook_path']).strip())
                has_audio = bool(item.get('lecture_audio_url') and str(item['lecture_audio_url']).strip())
                
                # ご提案通り、PDFか音声パスがあるものだけをリストに加える
                if has_pdf or has_audio:
                    results.append(item)
