...
def upload_db_background(local_path, remote_filename):
    ...
    if not os.path.exists(local_path):
        return
    thread = threading.Thread(target=upload_db_worker, args=(local_path, remote_filename))
    thread.start()
...
def upload_audio_background(local_filepath, remote_filename):
    thread = threading.Thread(target=upload_audio_worker, args=(local_filepath, remote_filename))
    thread.start()
